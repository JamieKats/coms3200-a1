...
timeout 1.1 bash -c "{ (sleep 0.3; echo 'Hi channel 1'; sleep 0.3; echo '/switch channel2'; sleep 0.3; echo 'Still here') | $(./decide.sh $1 client) $chan1port Suhas; }" > client-capture-A &
timeout 1.1 bash -c "{ sleep 0.1; $(./decide.sh $1 client) $chan1port Pragun; }"    > client-capture-B  &
timeout 1.1 bash -c "{ sleep 0.15; $(./decide.sh $1 client) $chan2port Suhas; }"    > client-capture-C &
sleep 1.5;
