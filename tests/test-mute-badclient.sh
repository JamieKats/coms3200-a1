...
timeout 2 bash -c "{ (sleep 0.3; echo '/mute channel1:nonexist 22') | $(./decide.sh $1 server) goodconf; }" > server-capture &
sleep 0.2; timeout 1 bash -c "{ (sleep 0.6; echo 'Not muted yay') | $(./decide.sh $1 client) $chan1port Dave; }"           > client-capture &
sleep 2.1;
