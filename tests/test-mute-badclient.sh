...
timeout 2.5 bash -c "{ ( sleep 0.7; echo '/mute channel1:nonexist 22') | $(./decide.sh $1 server) goodconf; }" > server-capture &
sleep 0.3; timeout 1.5 bash -c "{ ( sleep 0.9; echo 'Not muted yay') | $(./decide.sh $1 client) $chan1port Dave; }"           > client-capture &
sleep 2.6;
