...
timeout 0.9 bash -c "{ (sleep 0.3; echo '/thisisalsonotacommandsoyoushoulddonothing haha lol') | $(./decide.sh $1 client) $chan1port Chamith; }" > client-capture-A &
timeout 0.9 bash -c "{ $(./decide.sh $1 client) $chan1port Peter; }" > client-capture-B &
sleep 1;
