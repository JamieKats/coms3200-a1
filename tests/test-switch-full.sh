...
timeout 1.8 bash -c "{ (sleep 0.3; echo 'Hi channel 1'; sleep 0.3; echo '/switch channel2'; sleep 0.9; echo 'Hi channel 2') | $(./decide.sh $1 client) $chan1port Cody; }" > client-capture-A &
timeout 1.8 bash -c "{ sleep 0.1; $(./decide.sh $1 client) $chan2port full1; }"              > client-capture-B  &
timeout 1.8 bash -c "{ sleep 0.1; $(./decide.sh $1 client) $chan2port full2; }"              > /dev/null         &
timeout 1.8 bash -c "{ sleep 0.1; $(./decide.sh $1 client) $chan2port full3; }"              > /dev/null         &
timeout 1.8 bash -c "{ sleep 0.1; $(./decide.sh $1 client) $chan2port full4; }"              > /dev/null         &
timeout 1.2 bash -c "{ sleep 0.1; $(./decide.sh $1 client) $chan2port full5; }"              > /dev/null         &
timeout 1.2 bash -c "{ sleep 0.2; $(./decide.sh $1 client) $chan2port full6; }"   > /dev/null &
sleep 2.5;
