...
timeout 14 bash -c "{ $(./decide.sh $1 server) goodconf; }"                > server-capture    &
sleep 0.2; timeout 13 bash -c "{ (sleep 5; echo 'Hi channel 1'; sleep 0.3; echo '/switch channel2'; sleep 5.0; echo 'Hi channel 2') | $(./decide.sh $1 client) $chan1port Cody; }" > client-capture-A &
sleep 0.3; timeout 13 bash -c "{ $(./decide.sh $1 client) $chan2port full1; }"              > client-capture-B  &
sleep 0.4; timeout 13 bash -c "{ $(./decide.sh $1 client) $chan2port full2; }"              > /dev/null         &
sleep 0.5; timeout 13 bash -c "{ $(./decide.sh $1 client) $chan2port full3; }"              > /dev/null         &
sleep 0.6; timeout 13 bash -c "{ $(./decide.sh $1 client) $chan2port full4; }"              > /dev/null         &
sleep 0.7; timeout 5.8 bash -c "{ $(./decide.sh $1 client) $chan2port full5; }"              > /dev/null         &
sleep 0.8; timeout 6.0 bash -c "{ $(./decide.sh $1 client) $chan2port full6; }"   > /dev/null &
sleep 14.1;
